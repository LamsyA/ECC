...
use crate::{
    elliptic_curve::short_weierstrass::ecc_point::EllipticPoint,
    finite_field::field::{Field, FieldTrait},
};
...
pub fn main() {
    ...
    let a = Field::new(1, 13);
    let b = Field::new(0, 13);
    let curve = EllipticCurve::new(a, b);
    let x = Field::new(2, 13);
    let y = Field::new(6, 13);
    let point = EllipticPoint::new(x, y, curve);
    println!("{}", point);
}
